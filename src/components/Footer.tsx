...
import { Share2, Check, Heart, Code2, QrCode } from 'lucide-react';
...
export function Footer({ onOpenQr }: FooterProps) {
  ...
  return (
    <footer className="bg-stone-900 text-stone-300 py-10 px-4 sm:px-6 border-t border-stone-800 text-left">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 pb-6 border-b border-stone-800">
          ...
          {/* Action buttons: Share link, and under it - Scan QR code button */}
          <div className="flex flex-col gap-2.5 w-full sm:w-auto">
            <button
              id="btn-share-souvenir"
              onClick={handleShare}
              className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-gradient-to-r from-amber-600 to-rose-600 hover:from-amber-700 hover:to-rose-700 text-white text-xs font-bold flex items-center justify-center gap-2 shadow-md transition-all font-sans-ui cursor-pointer"
            >
            ...
            {onOpenQr && (
              <button
                ...
                onClick={onOpenQr}
                className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/40 text-xs font-bold flex items-center justify-center gap-2 shadow-sm transition-all font-sans-ui cursor-pointer"
              >
                <QrCode className="w-4 h-4 text-amber-400" />
                <span>
                  {isInternational ? 'Open / Scan QR Code' : 'Отсканировать QR-код сайта'}
                </span>
              </button>
            )}
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-xs text-stone-500 font-sans-ui pt-1">
          <div className="flex items-center gap-2">
